...
So clearly this model is getting us something. The only worry is that it's so slow. 

Let's try rewriting it so it is more vectorized?

### Attempt #3: Vectorized

OK, try not fitting word coefficients. Does this even make sense?

```{r}
mcmurray_vectorized <-'
data {                            
  int<lower=1> N; // number of datapoints (W*S)
  vector[N] age; // subject ages
  int<lower=0,upper=1> produces[N]; // actual data
}
...
model {
  mu_w ~ normal(0, 10); 
  mu_s ~ normal(0, 10);
  produces ~ bernoulli_logit(mu_s * age - mu_w);
}
'
```
...
dat <- list(age = model.data$age,
            produces = model.data$produces,
            W = n.words, 
            N = n.words * n.subs)

samps <- stan(model_code = mcmurray_vectorized, 
              model_name = "mcmurray_vectorized", 
              data = dat, iter = 1000, warmup=100, thin=10, chains = 4, 
              pars = c('mu_w','mu_s'), 
              verbose = F, refresh=2000/4) 
```