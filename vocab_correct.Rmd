...
parameters {
  real<lower=0> mu1; // word mean
  real<lower=0> mu2; // subject mean
  real<lower=0> sigma1; // word SD
  real<lower=0> sigma2; // subject SD
  vector[W] difficulty; // word mean
  vector[S] ability; // subject mean
}

model {
  difficulty ~ normal(mu1, sigma1);
  ability ~ normal(mu2, sigma2);
...
model.data <- ws %>%
  select(data_id, num_item_id, value, age) %>%
  filter(data_id < 10, num_item_id < 10) %>%
...
```{r}
# initializing unbounded at zero or setting the range for random init values proved useful
test <- stan(model_code = stanmodelcode, model_name = "mcmurray_simple", 
             init=0,  
             data = model.data, iter = 2000, warmup=200, thin=1, chains = 2, 
             pars = c('Intercept','betaCond','IntRE', 'SlopeRE', 
                      'phi', 'sd_int', 'sd_beta'), 
...
summary(samplerpar)
pairs(test, pars=c('Intercept', 'betaCond'))
...
p = proc.time()
parfit = parSapply(cl, 1:nchains, 
                   function(i) stan(model_code = stanmodelcode, 
                                    model_name = "mixedreg", 
                                    init=0,       # see comment in test run
                                    # init_r=.01, 
                                    fit = test, 
                                    data = dat, iter = iters, 
                                    warmup=wu, thin=thin, chains = 1, 
                                    chain_id=i,
...
### examine model
print(fit2, pars = c('Intercept','betaCond','IntRE', 
                     'SlopeRE', 'phi', 'sd_int', 'sd_beta', 'lp__'), 
      digits=4, probs = c(.025, .5, 0.975))

traceplot(fit2, inc_warmup=F, pars = c('Intercept','betaCond','IntRE',
                                       'SlopeRE', 'phi', 'sd_int', 
                                       'sd_beta', 'lp__'))
```